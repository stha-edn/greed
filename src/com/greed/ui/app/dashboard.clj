...
(defn- hero-substat [label value]
  [:div {:class "min-w-0"}
   [:p {:class "text-[11px] font-medium uppercase tracking-wider text-emerald-700/70 whitespace-nowrap"} label]
   [:p {:class "mt-1 text-sm sm:text-lg font-semibold tabular-nums text-zinc-900 whitespace-nowrap"} value]])
...
(defn- hero
  ...
  (let [{:finances/keys [salary payday]} finances
    ...
    [:div {:class "relative h-full overflow-hidden rounded-2xl p-6 sm:p-8 bg-gradient-to-br from-white via-emerald-50 to-emerald-100 ring-1 ring-emerald-200/70 shadow-card-md"}
     ...
     [:div {:class "relative flex h-full flex-col"}
      [:div {:class "flex items-start justify-between gap-3"}
       [:p {:class "text-xs font-medium uppercase tracking-widest text-emerald-700"} "Monthly net take-home"]
       (when payday
         [:span {:class "flex-shrink-0 text-xs font-medium text-emerald-700 bg-emerald-500/10 ring-1 ring-emerald-600/20 px-3 py-1 rounded-full"}
          (str "Payday · " (utilities/ordinal payday))])]
      [:p {:class "mt-3 text-4xl sm:text-5xl font-bold tracking-tight tabular-nums text-zinc-900"}
       (if monthly-net (utilities/amount->rands monthly-net) "—")]
      [:div {:class "mt-auto grid grid-cols-3 gap-2 sm:gap-4 border-t border-emerald-200/60 pt-5"}
