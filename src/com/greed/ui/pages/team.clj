...
(defn- team-member [{:keys [img name role delay]}]
  [:div {:class "flex-shrink max-w-full px-4 w-full sm:w-full md:w-2/3 lg:w-1/2 xl:px-6"}
   [:div (merge {:class "relative overflow-hidden bg-white mb-12 hover-grayscale-0 wow fadeInUp"
                 :data-wow-duration "1s"
                 :style (merge {:visibility "visible"
                                :animation-duration "1s"
                                :animation-name "fadeInUp"}
                          (when delay {:animation-delay delay}))}
           (when delay {:data-wow-delay delay}))
    [:div.relative.overflow-hidden.px-8
     [:img {:src img :class "aspect-square object-cover w-full mx-auto rounded-full bg-gray-50" :alt "title image"}]]
    [:div.pt-8.text-center
     [:p {:class "text-xl leading-normal font-bold mb-1"} name]
     [:p {:class "text-gray-500 leading-relaxed font-light text-base"} role]
     (social-icons)]]])

(defn page []
  [:div#team {:class "section relative pt-20 pb-8 md:pt-16 bg-white "}
   [:div {:class "container xl:max-w-6xl mx-auto px-4"}
    [:header {:class "text-center mx-auto mb-12"}
     [:h2 {:class "text-2xl leading-normal mb-2 font-bold text-gray-800"}
      [:span.font-light "Our"] " Team"]
     [:svg {"version" "1.1"
