...
(defn- guide []
  [:div {:class "space-y-6"}
   ...
   [:div {:class "bg-white rounded-lg shadow p-6"}
    ...
    [:div {:class "space-y-4"}
     [:div
      [:p {:class "text-sm font-medium text-zinc-800"} "Gross tax"]
      [:p {:class "text-xs text-zinc-500 mt-1"}
       "The raw tax calculated from the SARS brackets before any rebates are applied."]]
     [:div
      [:p {:class "text-sm font-medium text-zinc-800"} "Rebates"]
      [:p {:class "text-xs text-zinc-500 mt-1"}
     ...
     [:div
      [:p {:class "text-sm font-medium text-zinc-800"} "Effective tax rate"]
      [:p {:class "text-xs text-zinc-500 mt-1"}
